...
import { initLinkSavedDlqHandler } from "./link-saved-dlq-handler";
...
function createRecord(detail: unknown, receiveCount: number, messageId = "msg-1"): SQSRecord {
	return {
		...
		md5OfBody: "",
		eventSource: "aws:sqs",
		eventSourceARN: "arn:aws:sqs:ap-southeast-2:123456789:link-saved-dlq",
		awsRegion: "ap-southeast-2",
	};
}

function createSqsEvent(detail: { url: string; userId: string }, receiveCount = 3): SQSEvent {
	return { Records: [createRecord(detail, receiveCount)] };
}

describe("initLinkSavedDlqHandler", () => {
	it("dispatches markSummaryExhausted with the URL, reason, and receiveCount from the DLQ record", async () => {
		...
		const handler = initLinkSavedDlqHandler({
			transitionAndPersist,
			logger: noopLogger,
		});

		await handler(createSqsEvent({ url: "https://example.com/stuck", userId: "user-1" }, 4), stubContext, () => {});
	...
	it("reports the record as a batch failure when the transition throws", async () => {
		...
		const result = await handler(
			createSqsEvent({ url: "https://example.com/stuck", userId: "user-1" }, 4),
			stubContext,
			() => {},
		);
	...
	it("isolates per-record failures: only the failing record ends up in batchItemFailures", async () => {
		...
		const event: SQSEvent = {
			Records: [
				createRecord({ url: "https://example.com/ok", userId: "user-1" }, 4, "msg-ok"),
				createRecord({ url: "https://example.com/explode", userId: "user-1" }, 4, "msg-bad"),
			],
		};
