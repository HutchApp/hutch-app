...
import { initAnonymousLinkSavedDlqHandler } from "./anonymous-link-saved-dlq-handler";
...
function createRecord(detail: unknown, receiveCount: number, messageId = "msg-1"): SQSRecord {
	return {
		...
		md5OfBody: "",
		eventSource: "aws:sqs",
		eventSourceARN: "arn:aws:sqs:ap-southeast-2:123456789:anonymous-link-saved-dlq",
...
describe("initAnonymousLinkSavedDlqHandler", () => {
	it("dispatches markSummaryExhausted with the URL, reason, and receiveCount from the DLQ record", async () => {
		...
		const handler = initAnonymousLinkSavedDlqHandler({
			transitionAndPersist,
			logger: noopLogger,
		});

		await handler(createSqsEvent({ url: "https://example.com/stuck" }, 4), stubContext, () => {});
	...
	it("reports the record as a batch failure when the transition throws", async () => {
		...
		const result = await handler(
			createSqsEvent({ url: "https://example.com/stuck" }, 4),
			stubContext,
			() => {},
		);
