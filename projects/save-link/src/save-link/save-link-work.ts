...
export type ProcessContent = (params: {
	html: string;
...
export function initSaveLinkWork(deps: {
	...
	const saveLinkWork = async (url: string): Promise<SaveLinkWorkResult> => {
		...
		if (crawlResult.status === "unsupported") {
			...
			// to the terminal `unsupported` state — the aggregate transition pairs
			// crawl=unsupported with summary=skipped("crawl-unsupported") so the
			// canary doesn't keep flagging the row. No throw: this is a successful
			// terminal outcome, not work to retry.
