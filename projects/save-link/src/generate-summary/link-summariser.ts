...
export function initLinkSummariser(deps: {
	...
	const summarizeArticle: SummarizeArticle = async (params) => {
		...
		if (deps.isTooShortToSummarize(cleanedContent)) {
			deps.logger.info("[summarize] content too short, skipping", {
				url: params.url,
				visibleLength,
			});
		...
		if (!textBlock || textBlock.type !== "text" || !textBlock.text) {
			deps.logger.info("[summarize] no text block in response", {
				url: params.url,
			});
		...
		if (summary === "Summary not available.") {
			deps.logger.info("[summarize] AI returned unavailable", {
				url: params.url,
			});
