		...
		Records: [
			{
				...
				eventSource: "aws:sqs",
				eventSourceARN:
					"arn:aws:sqs:ap-southeast-2:123456789:GenerateGlobalSummary-dlq",
...
describe("initGenerateSummaryDlqHandler", () => {
	it("dispatches markSummaryExhausted with the receiveCount when a message lands in DLQ", async () => {
		...
		expect((transition as { name: string }).name).toBe("markSummaryExhausted");
