...
import { DeveloperPage } from "./web/pages/developer";
...
export function createApp(dependencies: AppDependencies): Express {
	...
	app.get("/sitemap.xml", (_req: Request, res: Response) => {
		const pages = [
			...
			{ loc: "/developer", priority: "0.8", changefreq: "monthly" },
	...
	app.get("/developer", async (req: Request, res: Response) => {
		...
		const browser: "firefox" | "chrome" | "other" =
			...
			: "other";
		const userCount = await countUsers().catch(() => 0);
		const result = DeveloperPage({ userCount, staticBaseUrl, browser }).to("text/html");
		res.status(result.statusCode).type("html").send(result.body);
	});
