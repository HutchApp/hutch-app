...
| `LinkSavedEvent { url, userId }` | `select-most-complete-content` Lambda (only on canonical change) | `link-saved` (vis 60s) → `link-saved` Lambda | auto-pair | (dispatches a command inline) | `GenerateSummaryCommand` directly to `generate-summary` queue (SQS `SendMessageCommand`, not EventBridge) — see [`../2026-04-20-52017f3/`](../2026-04-20-52017f3/) |
