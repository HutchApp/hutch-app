...
2. **HTTP-side publish failure leaves the row pending** — five of six entry points (`POST /queue`, `POST /queue/save`, `POST /queue/save-html`, `GET /view/<url>`, `GET /admin/recrawl/<url>`, `POST /import/:id/commit`) write the article row + `markCrawlPending` *before* awaiting `publishEvent`. If `PutEvents` throws, the catch (or unhandled error) returns a user-facing failure, but the pending row is never compensated. Class #1 by symptom, not class #2.
